 ...
 * @copyright abba yosua design 2019
 * @package gsja_tesalonika_app
 * 
 * 
...
/** push notification **/
$config["onesignal-appid"]			= "95a516ed-2cc5-44dc-a307-898e4705417c" ; 		//Your OneSignal AppId, available in OneSignal
$config["onesignal-appkey"]			= "OWE5NjMzYWUtMThlZi00YTdjLWIzNWYtMGE0YjM4NGFlMDMz" ; 		//Your OneSignal AppKey
/** CONFIG:END **/


/** LANGGUAGE:START **/
define("LANG_PUSH_NOTIFICATION","Push Notifications");
define("LANG_RENUNGAN","Renungans");
define("LANG_USER","Users");
define("LANG_ID","Ids");
define("LANG_JUDUL","Juduls");
define("LANG_GAMBAR","Gambars");
define("LANG_ISI_RENUNGAN","Isi Renungans");
...
if($_SESSION["IS_ADMIN"]==true){
	...
	$tags_html .= "</ul>" ;
	$tags_html .= "<h1 class=\"text-muted\">GSJA Tesalonika App</h1>" ;
	$tags_html .= "<p>Gsja Tesalonika App</p>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "<div class=\"container-fluid\">" ;
	...
	if($_GET["table"]=="renungans"){
		$tags_html .= "<li class=\"active\"><a href='?table=renungans'>". @LANG_RENUNGAN."</a></li>" ;
	}else{
		$tags_html .= "<li><a href='?table=renungans'>". @LANG_RENUNGAN."</a></li>" ;
	}
	if($_GET["table"]=="users"){
	...
	if($_GET["table"]=="push-notification"){
		$tags_html .= "<li class=\"active\"><a href='?table=push-notification'>".@LANG_PUSH_NOTIFICATION."</a></li>" ;
	}else{
		$tags_html .= "<li><a href='?table=push-notification'>".@LANG_PUSH_NOTIFICATION."</a></li>" ;
	}
	$tags_html .= "</ul>" ;
	$tags_html .= "</div>" ;
	$tags_html .= "</div>" ;
	...
	switch($_GET["table"]){
		case "home":
					$tags_html .= "<h4 class=\"page-title\">GSJA Tesalonika App</h4>" ;
					$tags_html .= "<div class=\"row\">" ;
					$tags_html .= "<div class=\"col-md-4\">" ;
					$tags_html .= "<div class=\"panel panel-primary\" >" ;
					$tags_html .= "<div class=\"panel-heading\">" ;
					$tags_html .= "<h4  class=\"panel-title\"><span class=\"glyphicon glyphicon-list-alt\"></span> ". @LANG_RENUNGAN ."</h4>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-body\" style=\"min-height: 150px;\">" ;
					$tags_html .= "<ul class=\"list\">" ;
					/** fetch data from renungan **/
					$sql_query = "SELECT * FROM `renungan` ORDER BY `id`  DESC LIMIT 0 , 5" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$tags_html .= "<li>" . htmlentities(stripslashes($data["judul"])) . "</li>" ;
						}
					}
					$tags_html .= "</ul>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"panel-footer text-right\"><a href=\"?table=renungans\" class=\"btn btn-sm btn-primary\">".LANG_MORE."</a></div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "</div>" ;
					$tags_html .= "<div class=\"col-md-4\">" ;
		...
		// TODO: Renungans
		case "renungans":
					$tags_html .= "<h4>". @LANG_RENUNGAN ."</h4>" ;
				switch($_GET["action"]){
					// TODO: ---- listing
					case "list":
					$tags_html .= "<ul class=\"nav nav-tabs\">" ;
					$tags_html .= "<li class=\"active\"><a href=\"?table=renungans\">".@LANG_LIST."</a></li>" ;
					$tags_html .= "<li><a href=\"?table=renungans&action=add\">".@LANG_ADD."</a></li>" ;
					$tags_html .= "</ul>" ;
					$tags_html .= "<br/>" ;
					...
					$table_html .= "<thead>" ;
					$table_html .= "<tr>" ;
					$table_html .= "<th>".@LANG_JUDUL."</th>" ;
					$table_html .= "<th>".@LANG_GAMBAR."</th>" ;
					$table_html .= "<th>".@LANG_ISI_RENUNGAN."</th>" ;
					$table_html .= "<th style=\"width:100px;\">Action</th>" ;
					$table_html .= "</tr>" ;
					$table_html .= "</thead>" ;
					$table_html .= "<tbody>" ;
					/** fetch data from mysql **/
					$sql_query = "SELECT * FROM `renungan`" ;
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							$table_html .= "<tr>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["judul"])) . "</td>" ;
							$table_html .= "<td><img src=\"" . $data["gambar"] . "\" alt=\"#\" width=\"64\" height=\"64\" /></td>" ;
							$table_html .= "<td>" . htmlentities(stripslashes($data["isi_renungan"])) . "</td>" ;
							$table_html .= "<td>" ;
							$table_html .= "<div class=\"btn-group\" >" ;
							if($config["onesignal-appkey"] !== ""){
								$table_html .= "<a target=\"_blank\" class=\"btn btn-sm btn-info\" href=\"?table=push-notification&specific=renungan_singles/". $data["id"]. "\"><span class=\"glyphicon glyphicon-bell\"></span></a> " ;
							}
							$table_html .= "<a class=\"btn btn-sm btn-warning\" href=\"?table=renungans&action=edit&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-pencil\"></span></a> " ;
							$table_html .= "<a onclick=\"return confirm('Are you sure you want to delete ID#". $data["id"]. "')\" class=\"btn btn-sm btn-danger\" href=\"?table=renungans&action=delete&id=". $data["id"]. "\"><span class=\"glyphicon glyphicon-trash\"></span></a> " ;
							$table_html .= "</div>" ;
							$table_html .= "</td>" ;
							$table_html .= "</tr>" ;
							$table_html .= "\r\n" ;
			...
			case "add":
				// TODO: ---- add
				$tags_html .= "<ul class=\"nav nav-tabs\">" ;
				$tags_html .= "<li><a href=\"?table=renungans&action=list\">".@LANG_LIST."</a></li>" ;
				$tags_html .= "<li class=\"active\"><a href=\"?table=renungans&action=add\">".@LANG_ADD."</a></li>" ;
				$tags_html .= "</ul>" ;
				$tags_html .= "<br/>" ;
				...
				if(isset($_POST["add"])){
					...
					if(isset($_POST["judul"])){
						$data_judul = addslashes($_POST["judul"]);
					}
					if(isset($_POST["gambar"])){
						$data_gambar = addslashes($_POST["gambar"]);
					}
				if(isset($_FILES["gambar-upload"]["name"])){
					if($_FILES["gambar-upload"]["name"]!=""){
						$ext = pathinfo($_FILES["gambar-upload"]["name"],PATHINFO_EXTENSION);
						$uploadfile =  "media/image/". sha1(time()).".".$ext;
						$uploadtemp =  $_FILES["gambar-upload"]["tmp_name"];
						...
						if(!is_dir(dirname(__FILE__)."/media/image/")) {
							...
							if (in_array(strtolower($ext),$config["image_allowed"])){
								if(preg_match("/image/",$mimetype["mime"])){
									...
									$data_gambar =  $full_url ."/". $uploadfile;
								}
							}
						}
					}
					if(isset($_POST["isi_renungan"])){
						$data_isi_renungan = addslashes($_POST["isi_renungan"]);
					}
	
					/** prepare save to mysql **/
					$sql_query = "INSERT INTO `renungan` (`judul`,`gambar`,`isi_renungan`) VALUES ('$data_judul','$data_gambar','$data_isi_renungan')" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=renungans&action=list");
				}
	
	
				/** Create Form **/
				$tags_html .= '
				<form id="renungan" action="" method="post" enctype="multipart/form-data">
...
<div class="form-group">
	<label for="judul">' . @LANG_JUDUL. '</label>
	<input  id="judul" class="form-control input-judul" data-type="heading-1" type="text" name="judul" placeholder="" />
...
-->
<div class="form-group">
	<label for="gambar">' . @LANG_GAMBAR. '</label>
	<textarea id="gambar" class="form-control input-gambar"  data-type="images" name="gambar" ></textarea>
	<p class="help-block">Using base64 image data (small images) <input data-target="#gambar" type="file" data-type="image-base64" /> or file upload <input name="gambar-upload" type="file" data-type="image-upload" /></p>
</div>


<!--
// TODO: ----|-- form : ISI_RENUNGAN
-->
<div class="form-group">
	<label for="isi_renungan">' . @LANG_ISI_RENUNGAN. '</label>
	<input  id="isi_renungan" class="form-control input-isi_renungan" data-type="text" type="text" name="isi_renungan" placeholder="" />
	<p class="help-block"></p>
</div>
					<div class="form-group">
						<label for="add"></label>
		...
		case "edit":
			...
			if(isset($_GET["id"])){
				...
				if(isset($_FILES["gambar-upload"]["name"])){
					...
					/** update data to sql **/
					$sql_query = "UPDATE `renungan` SET `judul` = '$data_judul',`gambar` = '$data_gambar',`isi_renungan` = '$data_isi_renungan' WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=renungans&action=list");
			...
			/** fetch current data **/
			$sql_query = "SELECT * FROM `renungan`  WHERE `id`=$entry_id LIMIT 0,1" ;
			if($result = $mysql->query($sql_query)){
				while ($data = $result->fetch_array()){
					$rows[] = $data;
				}
			...
			/** get single data **/
			if(isset($rows[0]["judul"])){
				$data_judul = stripslashes($rows[0]["judul"]) ;
			}
			if(isset($rows[0]["gambar"])){
				$data_gambar = stripslashes($rows[0]["gambar"]) ;
			}
			if(isset($rows[0]["isi_renungan"])){
				$data_isi_renungan = stripslashes($rows[0]["isi_renungan"]) ;
			}
	
			/** buat form edit **/
			$tags_html .= '
<form action="" method="post" enctype="multipart/form-data" id="renungan">
...
<div class="form-group">
	<label for="judul">' . @LANG_JUDUL. '</label>
	<input  id="judul" class="form-control input-judul" data-type="heading-1" type="text" name="judul" placeholder="" value="'.htmlentities($data_judul).'" />
	<p class="help-block"></p>
</div>
...
<div class="form-group">
	<label for="gambar">' . @LANG_GAMBAR. '</label>
	<textarea id="gambar" class="form-control input-gambar"  data-type="images" name="gambar" >'.htmlentities($data_gambar).'</textarea>
	<p class="help-block">Using base64 image data (small images) <input data-target="#gambar" type="file" data-type="image-base64" /> or file upload <input name="gambar-upload" type="file" data-type="image-upload" /></p>
</div>
...
<div class="form-group">
	<label for="isi_renungan">' . @LANG_ISI_RENUNGAN. '</label>
	<input  id="isi_renungan" class="form-control input-isi_renungan" data-type="text" type="text" name="isi_renungan" placeholder="" value="'.htmlentities($data_isi_renungan).'" />
	<p class="help-block"></p>
</div>
<div class="form-group">
	...
	<input class="btn btn-primary" type="submit" name="edit" />
			...
			// TODO: ---- delete
				...
				if(isset($_GET["id"])){
					...
					$entry_id = (int)$_GET["id"];
					/** delete item in sql **/
					$sql_query = "DELETE FROM `renungan` WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=renungans&action=list");
		...
		case "users":
				...
				switch($_GET["action"]){
					...
					if($result = $mysql->query($sql_query)){
						while ($data = $result->fetch_array()){
							...
							$table_html .= "<div class=\"btn-group\" >" ;
							if($config["onesignal-appkey"] !== ""){
								$table_html .= "<a target=\"_blank\" class=\"btn btn-sm btn-info\" href=\"?table=push-notification&specific=user_singles/". $data["id"]. "\"><span class=\"glyphicon glyphicon-bell\"></span></a> " ;
							}
			...
			case "add":
				...
				if(isset($_POST["add"])){
					...
					/** prepare save to mysql **/
					$sql_query = "INSERT INTO `user` (`fullname`,`uname`,`pwd`,`address`,`phone`,`email`) VALUES ('$data_fullname','$data_uname','$data_pwd','$data_address','$data_phone','$data_email')" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
		...
		case "edit":
			...
			if(isset($_GET["id"])){
				...
				if(isset($_POST["edit"])){
					...
					/** update data to sql **/
					$sql_query = "UPDATE `user` SET `fullname` = '$data_fullname',`uname` = '$data_uname',`pwd` = '$data_pwd',`address` = '$data_address',`phone` = '$data_phone',`email` = '$data_email' WHERE `id`=$entry_id" ;
					$stmt = $mysql->prepare($sql_query);
					$stmt->execute();
					$stmt->close();
					header("Location: ?table=users&action=list");
...
<div class="form-group">
	...
	<input class="btn btn-primary" type="submit" name="edit" />
		...
		$tags_html .= "<div class=\"container-fluid\">" ;
		$tags_html .= "<div class=\"row\">" ;
		$tags_html .= "<div class=\"col-md-12\"><p class=\"text-left navbar-text\">Copyright &copy; abba yosua design 2019 - All Rights Reserved</p></div>" ;
		$tags_html .= "</div>" ;
...
<html lang="en">
  <head>
    ...
    <title>GSJA Tesalonika App</title>    
